...
const insultGenerator = {
  ...
  generateInsult: function( insultCount=1 ){

    for( let i = 0; i < insultCount; i++ ){

      let firstAdjective  = this.getRandomElementFromArray( this.firstAdjectives );

      // Check if we've already used this first adjective, and keep choosing new ones until

      let retries = 0;
      while( firstAdjective in this.alreadyUsed ){
        console.log(`Found already used phrase: ${firstAdjective}`);
        firstAdjective  = this.getRandomElementFromArray( this.firstAdjectives );

        retries++;
        if( retries > this.firstAdjective.length ){
          console.log('Too many retries');
          this.alreadyUsed = {};  // this will cause the loop to exit at the start of the next iteration
        }
  ...
  generateEveryPossibleInsult: function(){
    ...
    for( let i = 0; i < this.firstAdjectives.length; i++ ){
      for( let j = 0; j < this.secondAdjectives.length; j++ ){
        for( let k = 0; k < this.nouns.length; k++ ){

          console.log(`Thou ${this.firstAdjectives[i]} ${this.secondAdjectives[j]} ${this.nouns[k]}!`);
          totalIterations++;
